...
namespace ETFSjedniceWeb
{
    public class StavkaDnevnogRedaController : Controller
    {
        ...
        string url = "http://servissjednice.azurewebsites.net/api/stavkadnevnogreda";
        ...
        {
            HttpResponseMessage responseMessage = await client.GetAsync("http://servissjednice.azurewebsites.net/api/dnevniRed" + "/" + 1);
            if (responseMessage.IsSuccessStatusCode)
            {
                var responseData = responseMessage.Content.ReadAsStringAsync().Result;
                DNEVNI_RED dnevniRed = JsonConvert.DeserializeObject<DNEVNI_RED>(responseData);
                tipGlasa.DNEVNI_RED = dnevniRed;
                responseMessage = await client.PostAsJsonAsync(url, tipGlasa);
                if (responseMessage.IsSuccessStatusCode)
                {
